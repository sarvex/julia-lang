...
mondo_regex = re.compile(
    f"^{c(arch_mapping)}{c(platform_mapping)}{c(libc_mapping)}{c(call_abi_mapping)}{c(libgfortran_version_mapping)}{c(cxx_abi_mapping)}$"
)

# Apply our mondo regex to our input:
m = mondo_regex.match(sys.argv[1])
if m is None:
    print(f"ERROR: Unmatchable platform string '{sys.argv[1]}'!")
    sys.exit(1)
